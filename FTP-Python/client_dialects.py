...
class D2:
    ...
    def do_get(self, args):
        ...
        try:
            ...
            ### June 29: update in dialect 2 <refer changes_dia.docx> ###
            ### send file size again ###
            recv_data = self.client_socket.recv(1024)
            ...
            if packet_info != "file not present":
                ...
                ### after 29 June changes, we get "123,123" here.
                ### earlier it was just "123" - the file size
                toks = packet_info.split(",")
                if len(toks) != 2 or toks[0] != toks[1] or not toks[0].isdigit():
                    dialect_mismatch("Expected int,int (same number 2 times)")
                    return
    ...
    def do_get(self, args):
        ...
        try:
            ...
            if packet_info[0] == "file present":
                ...
                file_size = 0
                try:
                    ### after 29 June changes, we get "file present,filesize,filename" here.
